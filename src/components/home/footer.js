import { Grid, Typography, Box } from "@mui/material";
import React from "react";
import FacebookIcon from '@mui/icons-material/Facebook';
import YouTubeIcon from '@mui/icons-material/YouTube';

const Footer = (props) => {
  return (
    <Grid container spacing={2} width={"100%"} height={"40vh"} bgcolor={"#F7F7F7"} fontSize={15} >
      <Grid xs={4} lg={4}>
        <Typography
          display={"flex"}
          justifyContent={"center"}
          alignItems={"center"}
          marginTop={"20vh"}
          color={"#7781AE"}
        >
          Чанар амлана
        </Typography>
      </Grid>
      <Grid xs={4} lg={3}>
        <Typography color={"#7781AE"} fontFamily={"bold"} marginTop={2} fontSize={15} >Танд хэрэгтэй холбоосууд</Typography>
        <Box color={"#FE5A03"} marginTop={2} marginLeft={8}  >
        <Typography fontSize={12} marginTop={1} >Компьютер угсрах</Typography>
        <Typography fontSize={12} marginTop={1} >Хэрхэн PC Format хийх вэ?</Typography>
        <Typography fontSize={12} marginTop={1} >Ryzen CPU гэж юу вэ?</Typography>
        <Typography fontSize={12} marginTop={1} >Intel CPU гэж юу вэ?</Typography>
        <Typography fontSize={12} marginTop={1} >Хэрхэн зөв PSU сонгох вэ?</Typography>
        <Typography fontSize={12} marginTop={1} >Таны компьютерт хэрэгтэй 5 зөвлөгөө</Typography>
        </Box>
      </Grid>
      <Grid xs={4} lg={5}>
      <Box
      marginTop={2}
          component="img"
          src="data:image/png;base64,"
          alt="{main image}"
          sx={{
            height: "2.6rem",
            width: "8rem",
            marginLeft: "3rem",
            marginRight: "3rem",
            cursor: "pointer",
          }}
        />
        <Box marginLeft={6} >
        <Typography color={"#7781AE"} fontSize={12}>2020 © Зохиогчийн эрх хуулиар хамгаалагдав </Typography>
         <Box display={"flex"} justifyContent={"flex-start"} >
          <Typography fontSize={12} color={"#7781AE"} >Хөгжүүлсэн </Typography>
          <Typography fontSize={12} color={"#FE5A03"} marginLeft={1}> HACKUM УБ,</Typography>
          </Box>
          <Typography fontSize={12} color={"#7781AE"} marginBottom={3}>БАЯНБҮРД, КОМПЬЮТЕР МОЛЛ, 2020 ТООТ</Typography>
          <Typography fontSize={12} color={"#FE5A03"} >Холбоо барих Түгээмэл асуултууд</Typography>
          <Typography fontSize={12} color={"#7781AE"} >Холбоосууд</Typography>
          <Box>
            <FacebookIcon></FacebookIcon>
            <YouTubeIcon></YouTubeIcon>
          </Box>
          </Box>
      </Grid>
    </Grid>
  );
};

export default Footer;
